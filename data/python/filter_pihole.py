...
输出: 
  - adblock_pihole_black.txt (黑名单规则)
  - adblock_pihole_white.txt (白名单规则)
功能:
  1. 分别处理黑白名单文件
  2. 严格过滤对应语法的规则
  3. 自动去重处理
...
BLACK_OUTPUT = "adblock_pihole_black.txt"
WHITE_OUTPUT = "adblock_pihole_white.txt"
...
def process_file(input_path: Path, is_blacklist: bool) -> set:
    """处理输入文件并返回转换后的规则集合"""
...
def write_output(rules: set, output_path: Path, is_blacklist: bool):
    """将规则写入输出文件"""
    ...
    rule_type = "黑名单" if is_blacklist else "白名单"
    logger.info(f"生成 {output_path.name}: {len(rules)} 条{rule_type}规则")
...
def main():
    ...
    write_output(black_rules, repo_root / BLACK_OUTPUT, is_blacklist=True)
    
    # 处理白名单
    white_rules = process_file(repo_root / WHITE_INPUT, is_blacklist=False)
    write_output(white_rules, repo_root / WHITE_OUTPUT, is_blacklist=False)
