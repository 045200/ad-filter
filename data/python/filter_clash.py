...
class Config:
    ...
    # 输出文件
    OUTPUT_CLASH_BLOCK = os.path.join(BASE_DIR, "adblock_clash_block.yaml")
    OUTPUT_CLASH_ALLOW = os.path.join(BASE_DIR, "adblock_clash_allow.yaml")
    OUTPUT_SURGE_BLOCK = os.path.join(BASE_DIR, "surge_blacklist.conf")
    OUTPUT_SURGE_ALLOW = os.path.join(BASE_DIR, "surge_whitelist.conf")
    # Mihomo配置
...
def convert_to_clash(rules: List[Tuple[str, str, str]]) -> Tuple[List[str], List[str]]:
    ...
    # 使用集合来去重
    block_rules_set: Set[str] = set()
    allow_rules_set: Set[str] = set()
    ...
    for rule_type, target, action in rules:
        ...
        if action == "REJECT":
            block_rules_set.add(clash_rule)
        else:
            allow_rules_set.add(clash_rule)
    
    # 转换为有序列表
    clash_block.extend(sorted(block_rules_set))
    clash_allow.extend(sorted(allow_rules_set))
...
def convert_to_surge(rules: List[Tuple[str, str, str]]) -> Tuple[List[str], List[str]]:
    ...
    for rule_type, target, action in rules:
        ...
        if action == "REJECT":
            block_rules_set.add(surge_rule)
        else:
            allow_rules_set.add(surge_rule)
    
    # 转换为有序列表
    surge_block.extend(sorted(block_rules_set))
    surge_allow.extend(sorted(allow_rules_set))
