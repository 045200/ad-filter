...
class Config:
    ...
    OUTPUT_FILE = OUTPUT_DIR / "adblock_clash.yaml"  # 拦截规则
    ALLOW_FILE = OUTPUT_DIR / "allow_clash.yaml"     # 允许规则
...
class RegexPatterns:
    ...
    CLASH_RULE = re.compile(r'^(DOMAIN-SUFFIX|DOMAIN|DOMAIN-KEYWORD|IP-CIDR).+,.+$')  # Clash原生规则


def setup_logger():
    logger = logging.getLogger('ClashMerger')
...
class ClashMerger:
    ...
    def run(self):
        ...
        if os.getenv('GITHUB_ACTIONS') == 'true':
            logger.info(f"::set-output name=clash_file::{Config.OUTPUT_FILE}")
            logger.info(f"::set-output name=clash_allow_file::{Config.ALLOW_FILE}")
    ...
    def _process_line(self, line: str, stats: Dict) -> Tuple[str, bool]:
        ...
        # 转换为Clash规则
        clash_rule, is_allow = self._to_clash(line)
        if not clash_rule:
            stats['unsupported'] += 1
            return None, False

        stats['valid'] += 1
        return clash_rule, is_allow

    def _to_clash(self, line: str) -> Tuple[str, bool]:
        # 白名单规则（@@开头）
        if line.startswith('@@'):
            normalized = line[2:]
