...
class AdvancedRuleParser:
    ...
    # AdGuard修饰符（新增AdGuard Home专属修饰符）
    ADGUARD_MODIFIERS = {
        ...
        'dnstype', 'dnsrewrite', 'client'  # 新增AdGuard Home修饰符
    }
    ...
    def validate_rule(self, rule: str) -> bool:
        ...
        try:
            ...
            if '$dnsrewrite=' in rule:
                target = rule.split('$dnsrewrite=')[1].strip()
                try:
                    ipaddress.ip_address(target)
                    self.rule_stats['valid_rules'] += 1
                    return True
                except:
                    logger.debug(f"无效DNS重写目标: {target}")
                    self.rule_stats['invalid_rules'] += 1
                    return False

            # 新增dnstype验证
            if '$dnstype=' in rule:
                dnstype = rule.split('$dnstype=')[1].split('|')
                valid_types = {'A', 'AAAA', 'CNAME', 'MX', 'TXT', 'SRV', 'PTR'}
                for t in dnstype:
                    if t.strip().upper() not in valid_types:
                        logger.debug(f"无效dnstype值: {t}")
                        self.rule_stats['invalid_rules'] += 1
                        return False

            # 新增client验证
            if '$client=' in rule:
                clients = rule.split('$client=')[1].split('|')
                for client in clients:
                    if not re.match(r'^[a-zA-Z0-9_-]+$', client.strip()):
                        logger.debug(f"无效client名称: {client}")
                        self.rule_stats['invalid_rules'] += 1
                        return False
    ...
    def parse_regex_rule(self, rule: str) -> Optional[str]:
        """解析正则表达式规则"""
        if self.is_regex_rule(rule):
            try:
                re.compile(rule[1:-1])  # 验证正则表达式有效性
                return rule
            except re.error as e:
                logger.debug(f"无效正则表达式规则: {rule}, 错误: {e}")
        return None

    def parse_dns_rewrite(self, rule: str) -> Optional[str]:
        """解析DNS重写规则"""
        if '$dnsrewrite=' in rule:
            parts = rule.split('$dnsrewrite=')
            target = parts[1].strip()
            try:
                ipaddress.ip_address(target)  # 验证IP格式
                return f"{parts[0].strip()}$dnsrewrite={target}"
            except:
                logger.debug(f"无效DNS重写目标: {target}")
        return None
    ...
    def classify_rule(self, line: str) -> Tuple[Optional[str], bool]:
        ...
        # 4. 处理DNS重写规则
        rewritten_rule = self.parse_dns_rewrite(original_line)
        if rewritten_rule:
            return rewritten_rule, is_allow

        # 5. 处理正则表达式规则
        regex_rule = self.parse_regex_rule(original_line)
        if regex_rule:
            return regex_rule, is_allow
...
class AdvancedRuleMerger:
    ...
    def remove_conflicts(self):
        ...
        for rule in self.allow_rules:
            ...
            elif self.parser.is_regex_rule(rule):
                try:
                    allow_regexes.append(re.compile(rule[1:-1]))
                except re.error:
                    pass
        ...
        for rule in self.block_rules:
            ...
            # 处理正则规则
                ...
                try:
                    regex = re.compile(rule[1:-1])
                    for allow_regex in allow_regexes:
                        if allow_regex.search(regex.pattern):
                            rules_to_remove.add(rule)
                            break
                except re.error:
                    pass
