...
try:
    ...
    print(f"缺少必要的依赖库: {e}")
    print("请使用 pip install adblockparser IPy pybloomfiltermmap3 安装")
    exit(1)
...
class EnhancedRuleParser:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("布隆过滤器初始化失败，使用集合去重")
    ...
    for file_path in [output_block, output_allow]:
        if file_path.exists():
            ...
            try:
                ...
                logger.error(f"读取现有规则文件 {file_path} 时出错: {e}")
...
def write_rules(block_rules: List[str], allow_rules: List[str]):
    ...
    try:
        with open(Config.OUTPUT_DIR / Config.OUTPUT_BLOCK, 'w', encoding='utf-8', newline='\n') as f:
            f.write("! Title: Merged AdBlock Filter Rules\n")
            f.write("! Description: Combined and deduplicated adblock rules from multiple sources\n")
            f.write("! Last Updated: " + datetime.now().isoformat() + "\n")
            f.write("! Rule Count: " + str(len(block_rules)) + "\n\n")
    ...
    try:
        with open(Config.OUTPUT_DIR / Config.OUTPUT_ALLOW, 'w', encoding='utf-8', newline='\n') as f:
            f.write("! Title: Merged Allow Rules\n")
            f.write("! Description: Combined and deduplicated allow rules from multiple sources\n")
            f.write("! Last Updated: " + datetime.now().isoformat() + "\n")
            f.write("! Rule Count: " + str(len(allow_rules)) + "\n\n")
