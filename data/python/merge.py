...
class AdGuardSyntaxDatabase:
    ...
    def load_syntax_database(self):
        ...
        possible_paths = [self.config.SYNTAX_DB_FILE, self.config.BASE_DIR / "adblock_syntax_db.json", Path(__file__).parent / "adblock_syntax_db.json"]
        ...
        try:
            ...
            self.allowed_dns_record_types = db_data.get('common_patterns', {}).get('adguard_home_dns_rewrite_types', self.config.ALLOWED_DNS_RECORD_TYPES)
    ...
    def validate_database_integrity(self, db_data: Dict) -> bool:
        ...
        # 验证DNS记录类型字段格式
        if not isinstance(db_data.get('common_patterns', {}).get('adguard_home_dns_rewrite_types', []), list):
...
class EnhancedBloomFilter:
    ...
    def __init__(self, config: AdGuardConfig):
        self.config = config
        self.bloom = ScalableBloomFilter(initial_capacity=config.BLOOM_INIT_CAP, error_rate=config.BLOOM_ERROR_RATE, mode=ScalableBloomFilter.LARGE_SET_GROWTH)
        self.hash_set = set()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {"total_items": total, "false_positive_count": self.false_positive_count, "false_positive_rate": self.false_positive_count / total if total > 0 else 0}
...
class AdGuardMerger:
    ...
    def __init__(self, config: AdGuardConfig):
        ...
        self.stats = {
            ...
            "duplicates": 0, "invalid_rules": 0, "rfc_violation_rules": 0,  # 新增：RFC违规数
            "adhome_compatible_rules": 0, "adhome_incompatible_rules": 0, "normalized_rules": 0
        }
    ...
    def validate_ipv4(self, ipv4: str) -> Tuple[bool, str]:
        ...
        # 2. 排除无效地址（如0.0.0.0用于拦截，特殊允许；但255.255.255.255广播地址禁止）
    ...
    def validate_ipv6(self, ipv6: str) -> Tuple[bool, str]:
        ...
        """
        ipv6 = ipv6.strip()
        # 1. 简化验证：允许零压缩（::），但禁止重复压缩、前导零（RFC 5952）
        # 参考Python ipaddress模块逻辑（避免依赖第三方库）
        try:
            # 替换零压缩为临时段（8段）
            if '::' in ipv6:
                if ipv6.count('::') > 1:
    ...
    def validate_dns_rewrite(self, rewrite_rule: str) -> Tuple[bool, str]:
        ...
        支持格式：$dnsrewrite=A;example.com;192.168.1.1 或 $dnsrewrite=CNAME;test.com;example.com
        返回：(是否合规, 违规原因)
        """
        # 提取DNS重写内容（如"A;example.com;192.168.1.1"）
    ...
    def analyze_rule_syntax(self, rule: str) -> Dict[str, Any]:
        ...
        # 5. 正则规则（提取域名/IP片段验证，RFC 1034/791）
            ...
            if violations:
                ...
                result['is_valid'] = True  # 正则本身语法不验证（复杂度高）

        # 6. 其他规则类型（默认无效，除非非严格模式）
        else:
            result['is_valid'] = not self.config.STRICT_RFC_VALIDATION  # 非严格模式允许未知类型
    ...
    def normalize_rule(self, rule: str) -> Optional[str]:
        ...
        if analysis['pattern_type'] in ['domain_rule', 'adguard_domain_rule']:
            ...
            # 修复：清理连续连字符（如do--main.com → do-main.com）
            if '--' in normalized:
                normalized = re.sub(r'--+', '-', normalized)
                self.github_log('debug', f"修复域名连续连字符：{analysis['normalized']} → {normalized}")
        ...
        for rule in batch:
            ...
            if not analysis['is_valid']:
                ...
                if analysis['rfc_violation_msg']:
                    self.github_log('debug', f"RFC违规规则（跳过）: {rule[:50]}... → 原因：{analysis['rfc_violation_msg']}")
                else:
    ...
    def print_statistics(self):
        ...
        logger.info(f"RFC违规规则: {self.stats['rfc_violation_rules']}条（不符合DNS/IP国际标准）")
        ...
        if self.config.GITHUB_ACTIONS and os.getenv('GITHUB_STEP_SUMMARY'):
            with open(os.getenv('GITHUB_STEP_SUMMARY'), 'a', encoding='utf-8') as f:
                f.write(f"""## AdGuard规则处理报告（含RFC标准验证）
- **处理时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
- **RFC严格验证**: {'开启' if self.config.STRICT_RFC_VALIDATION else '关闭'}
- **有效规则总数**: {self.stats['adguard_block_rules'] + self.stats['adguard_allow_rules']}
- **RFC违规规则**: {self.stats['rfc_violation_rules']}条（不符合DNS/IP国际标准）
- **AdGuard Home兼容率**: {self.stats['adhome_compatible_rules']/(self.stats['adguard_block_rules'] + self.stats['adguard_allow_rules']):.2%}
- **输出文件**: 
...
""")
    ...
    # -------------------------- 原有方法（get_files_by_prefix/process_file_batch/save_results）保留 --------------------------
    def get_files_by_prefix(self, directory: Path) -> Tuple[List[Path], List[Path]]:
        ...
        self.file_stats = {"total_files": len(block_files) + len(allow_files), "block_files": len(block_files), "allow_files": len(allow_files)}
...
def main():
    ...
    if config.GITHUB_ACTIONS:
        logger.info(f"运行环境: GitHub Actions（RFC严格验证: {'开启' if config.STRICT_RFC_VALIDATION else '关闭'}）")
