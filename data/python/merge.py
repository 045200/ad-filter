...
class AdblockMerger:
    ...
    def _process_file(self, file_path: Path) -> Tuple[List[str], Dict]:
        ...
        for chunk in file_chunk_reader(file_path):
            for line in chunk:
                ...
                rule = self._to_adblock(line, stats)
                if rule:
                    rules.append(rule)
    ...
    def _empty_stats() -> Dict:
        return {
            'total': 0, 'filtered': 0, 'unsupported': 0,
            'converted_hosts': 0, 'converted_plain': 0, 'native_adblock': 0
        }
